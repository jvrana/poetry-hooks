...
def new_project(
    ...
    Callable[..., ContextManager[LocalPath]], Callable[..., ContextManager[dict]]
]:
    """.. code-block::
    ...
    toml_ctx = functools.partial(
        create_pyproject, template=fixtures.join("fake_pyproject.toml")
    )
    return proj_ctx, toml_ctx


@pytest.mark.parametrize("git", [True, False])
@pytest.mark.parametrize("pkg", ["pkg1", "pkg2"])
